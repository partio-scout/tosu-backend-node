...
beforeEach(async () => {
  ...
  otherTosu = await models.Tosu.create({
...
test('Cannot delete when not logged in', async () => {
  await api.delete('/activities/5').expect(403)
})
...
test('Move activity from event to buffer', async () => {
  ...
  await api
    .put('/activities/' + activity.id + '/tobuffer')
    .set('cookie', [cookie])
...
test('Cannot move activity from event to buffer when not logged in', async () => {
  await api.put('/activities/666/tobuffer').expect(403)
})
...
test('Move activity from buffer to event', async () => {
  ...
  await api
    .put('/activities/' + activity.id + '/toevent/' + event.id)
    .set('cookie', [cookie])
    .expect('Content-Type', /json/)
    .then(result => {
      // Returned activity is correct
...
test('Cannot move activity from buffer to event when scout does not own the buffer', async () => {
  await models.ActivityBuffer.create({ scoutId: scout.id })
  const otherScoutsBuffer = await models.ActivityBuffer.create({
    scoutId: otherScout.id
  })
  const activity = await models.Activity.create({
    guid: 'vmp',
    activityBufferId: otherScoutsBuffer.id
  })
...
test('Cannot move activity from buffer to event when event does not exist', async () => {
  ...
  await api
    .put('/activities/' + activity.id + '/toevent/' + deletedEventId)
    .set('cookie', [cookie])
    .expect(500)
...
test('Cannot move activity from buffer to event when not logged in', async () => {
  await api.put('/activities/999/toevent').expect(403)
})
...
test('Add plan to activity', async () => {
  ...
  await api
    .post('/activities/' + activity.id + '/plans')
    .set('cookie', [cookie])
    .send(plan)
    .expect('Content-Type', /json/)
    .expect(200)
...
test('Cannot add plan to activity when not logged in', async () => {
  await api.post('/activities/666/tobuffer').expect(403)
})
